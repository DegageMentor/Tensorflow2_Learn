...
class Generator(keras.Model):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        x = tf.nn.leaky_relu(x)                                             # (b, 3, 3, 512)

        x = tf.nn.leaky_relu(self.bn1(self.conv1(x), training=training))    # (b, 9, 9, 256)

        x = tf.nn.leaky_relu(self.bn2(self.conv2(x), training=training))    # (b, 21, 21, 128)

        x = self.conv3(x)   # (b, 64, 64, 3)
...
class Discriminator(keras.Model):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        x = tf.nn.leaky_relu(self.conv1(inputs))    # (b, 20, 20, 64)

        x = tf.nn.leaky_relu(self.bn2(self.conv2(x), training=training))    # (b, 6, 6, 128)

        x = tf.nn.leaky_relu(self.bn3(self.conv3(x), training=training))    # (b, 1, 1, 256)
